...
func (s *StatsTab) buildChart(stats []AggregatedStats) fyne.CanvasObject {
	...
	// Draw zero line
	totalWidth := float32(len(stats)) * (barWidth + spacing)
	if totalWidth < 300 {
		totalWidth = 300
	}
	zeroLine := canvas.NewLine(zeroLineColor)
	...
	for i, st := range stats {
		...
		if net != 0 {
			...
			barHeight := float32(net) / float32(maxAbs) * halfHeight
			if barHeight < 0 {
				barHeight = -barHeight
			}
			...
			if net > 0 {
				...
				bar = canvas.NewRectangle(winColor)
				yPos = halfHeight - barHeight
			} else {
				// Red bar below zero line
				bar = canvas.NewRectangle(lossColor)
				yPos = halfHeight
			}
			...
			// Net value label on bar - centered horizontally and vertically
			netLabel := canvas.NewText(fmt.Sprintf("%+d", net), color.White)
			netLabel.TextSize = 10
			netLabel.Alignment = fyne.TextAlignCenter
			// Estimate text width based on character count (roughly 8px per char at size 10)
			labelText := fmt.Sprintf("%+d", net)
			textWidth := float32(len(labelText)) * 8
			textHeight := float32(12) // approximate height for size 10 font
		...
		dateLabel.Move(fyne.NewPos(xOffset, chartHeight+5))
		bars = append(bars, dateLabel)
	}

	// Legend - use canvas text for consistent alignment
	...
	chartCanvas := container.NewWithoutLayout(bars...)
	chartCanvas.Resize(fyne.NewSize(totalWidth, chartHeight+25))

	scrollable := container.NewHScroll(chartCanvas)
	scrollable.SetMinSize(fyne.NewSize(300, chartHeight+50))
